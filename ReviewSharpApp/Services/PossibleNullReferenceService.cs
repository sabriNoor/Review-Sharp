...
namespace ReviewSharp.Services
{
    public class PossibleNullReferenceService : ICodeReviewService
    {
        public List<CodeReviewResult> Review(CompilationUnitSyntax root)
        {
            ...
            results.AddRange(CheckMemberAccessesForNull(memberAccesses, allNullableNames));
            ...
            results.AddRange(CheckNullableUsageInExpressions(binaryExpressions, allNullableNames));

            var assignments = root.DescendantNodes().OfType<AssignmentExpressionSyntax>();
            results.AddRange(CheckNullableUsageInAssignments(assignments, allNullableNames));

            var returns = root.DescendantNodes().OfType<ReturnStatementSyntax>();
            results.AddRange(CheckNullableUsageInReturns(returns, allNullableNames));

            return results;
        }
        private static IEnumerable<CodeReviewResult> CheckNullableUsageInExpressions(IEnumerable<BinaryExpressionSyntax> expressions, HashSet<string> nullableNames)
        {
            foreach (var expr in expressions)
            {
                foreach (var side in new[] { expr.Left, expr.Right })
                {
                    if (side is IdentifierNameSyntax id && nullableNames.Contains(id.Identifier.Text))
                    {
        ...
        private static IEnumerable<CodeReviewResult> CheckNullableUsageInAssignments(IEnumerable<AssignmentExpressionSyntax> assignments, HashSet<string> nullableNames)
        {
            foreach (var assign in assignments)
            {
                if (assign.Right is IdentifierNameSyntax id && nullableNames.Contains(id.Identifier.Text))
                {
        ...
        private static IEnumerable<CodeReviewResult> CheckNullableUsageInReturns(IEnumerable<ReturnStatementSyntax> returns, HashSet<string> nullableNames)
        {
            foreach (var ret in returns)
            {
                if (ret.Expression is IdentifierNameSyntax id && nullableNames.Contains(id.Identifier.Text))
                {
        ...
        private static IEnumerable<CodeReviewResult> CheckMemberAccessesForNull(IEnumerable<MemberAccessExpressionSyntax> memberAccesses, HashSet<string> nullableNames)
        {
            foreach (var memberAccess in memberAccesses)
            {
                if (memberAccess.Expression is IdentifierNameSyntax id)
                {
                    if (nullableNames.Contains(id.Identifier.Text))
                    {
