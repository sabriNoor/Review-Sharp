...
namespace ReviewSharp.Tests
{
    public class PossibleNullReferenceServiceTests
    {
        ...
        [Fact]
        public void Review_NullableCheckedWithNotNull_NoWarning()
        {
        ...
        [Fact]
        public void Review_NullableCheckedWithIsNotNull_NoWarning()
        {
            var code = "class C { public int? p { get; set; } void M() { if(p is not null) { int y = p + 1; } } }";
            var results = GetResults(code);
            Assert.DoesNotContain(results, r => r.Message.Contains("Possible null reference on 'p'"));
        }
        ...
        [Fact]
        public void Review_NullableCheckedWithElseBlock_Warns()
        {
            var code = "class C { public int? p { get; set; } void M() { if(p != null) { int y = p + 1; } else { int z = p + 2; } } }";
            var results = GetResults(code);
            Assert.Contains(results, r => r.Message.Contains("Possible null reference on 'p'"));
        }
        ...
        [Fact]
        public void Review_NullableCheckedWithNullCoalescing_NoWarning()
        {
            var code = "class C { public int? p { get; set; } void M() { int y = p ?? 0; } }";
            var results = GetResults(code);
            Assert.DoesNotContain(results, r => r.Message.Contains("Possible null reference on 'p'"));
        }

        [Fact]
        public void Review_NullableCheckedWithTernary_NoWarning()
        {
            var code = "class C { public int? p { get; set; } void M() { int y = p != null ? p.Value : 0; } }";
            var results = GetResults(code);
            Assert.DoesNotContain(results, r => r.Message.Contains("Possible null reference on 'p'"));
        }

        [Fact]
        public void Review_NullableCheckedWithThrowIfNull_NoWarning()
        {
            var code = "class C { public int? p { get; set; } void M() { int y = p ?? throw new System.Exception(); } }";
            var results = GetResults(code);
            Assert.DoesNotContain(results, r => r.Message.Contains("Possible null reference on 'p'"));
        }
